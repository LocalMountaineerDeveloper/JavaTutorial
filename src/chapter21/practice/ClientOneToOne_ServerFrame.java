/*
 * 可选择用户的一对一聊天程序——服务端：一个客户端可以指定与另一个客户端通信。
 * */
...
public class ClientOneToOne_ServerFrame extends JFrame
{
    ...
    private Hashtable<String,Socket> hashtable=new Hashtable<String,Socket>();// 用于存储连接到服务器的用户和客户端套接字对象
    ...
    class ServerThread extends Thread
    {
        ...
        public void run()
        {
            try
            {
                ...
                while(true)
                {
                    String info=bufferedreaderIn.readLine();// 读取信息
                    String key="";
                    if(info.startsWith("用户："))
                    {// 添加登录用户到客户端列表
                        key=info.substring(3,info.length());// 获得用户名并作为键使用
                        hashtable.put(key,socket);// 添加键值对
                        Set<String> set=hashtable.keySet();// 获得集合中所有键的Set视图
                        Iterator<String> keyIt=set.iterator();// 获得所有键的迭代器
                        while(keyIt.hasNext())
                        {
                            String receiveKey=keyIt.next();// 获得表示接收信息的键
                            Socket s=hashtable.get(receiveKey);// 获得与该键对应的套接字对象
                            PrintWriter printwriterOut=new PrintWriter(s
                            ...
                            while(keyIt1.hasNext())
                            {
                                ...
                                printwriterOut.println(receiveKey1);// 发送信息
                    ...
                    {// 转发接收的消息
                        ...
                        while(keyIt.hasNext())
                        {
                            ...
                            {// 如果是发送方，但不是用户本身
                                ...
                                printwriterOut.println("MSG:"+info);// 发送信息
                                printwriterOut.flush();// 刷新输出缓冲区
                            }
                        }
                    }
